...
#                await button.click()
                ...
                counter = counter + 1
                if counter == 30:
                    SendEmail("Etrade2: Unable to login for 30 seconds")
                    sys.exit()

#################################################################################################################################

        for NumberOfAccounts in range(1,3):
            tryloop = True
            counter = 0
            while tryloop == True:
                try:
...
#                    await asyncio.wait([element.click(), element.waitForNavigation()])
                    ...
                    counter = counter + 1
                    if counter == 60:
                        SendEmail("Etrade2: Unable to get 1st account number for 30 seconds")
                        sys.exit()
...
#        await element.click()
            ...
            while tryloop == True:
                ...
                try:
                    ...
                    counter = counter + 1
                    if counter == 60:
                        SendEmail("Etrade2: Unable to get 1st list of funds page for 30 seconds")
                        sys.exit()
            ...
            while tryloop == True:
                try:
                    ...
                    counter = counter + 1
                    if counter == 60:
                        SendEmail("Etrade2: Unable to read 1st set of transactions for 30 seconds")
                        sys.exit()
            ...
            for iTrans in range(1, datapoints, 2):
                ...
                TransRecord = "'" + TransDate.strftime("%Y-%m-%d") + "','" + AcctNumber + "','" + TransDesc + "','" + TransType + "','" + TransAmount.replace(',','') + "'"
...
#******************************************************************************************
        ...
        while tryloop == True:
            try:
                ...
                counter = counter + 1
                if counter == 30:
                    SendEmail("Etrade2: Unable to read date information for 30 seconds")
                    sys.exit()
...
def WriteData(response, dateStr):
    ...
    for data in response:
        ...
        TotalValue = TotalValue + float(Shares) * float(Price)
...
def CheckTotals(Total, TotalValue):
...
try:
    SQLStmt = "SELECT * FROM Financial.Accounts WHERE Account_Name='Etrade2' AND Account_Type='Investment' ORDER BY Account_Name, Account_Username"
    cursor.execute(SQLStmt)
